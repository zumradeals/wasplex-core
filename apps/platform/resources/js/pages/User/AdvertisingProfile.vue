...
<template>
    ...
    <main class="min-h-screen bg-[#03101e] text-white">
        <div
            class="mx-auto min-h-screen max-w-3xl border-x border-white/5 bg-wasplex-night shadow-2xl"
        >
            <header
            ...
            <section class="px-5 py-7 sm:px-8">
                <div
                    class="rounded-[2rem] border border-wasplex-cyan/15 bg-gradient-to-br from-wasplex-blue/55 to-white/[0.035] p-6 sm:p-8"
                >
                    <div class="flex flex-wrap items-start justify-between gap-5">
                        <div class="max-w-xl">
                            <p
                                class="text-xs font-black tracking-[0.2em] text-wasplex-cyan uppercase"
                            >
                                Vos choix évoluent avec vous
                            </p>
                            <h1 class="mt-3 text-3xl font-black tracking-tight sm:text-4xl">
                                Mon Profil intelligent
                            </h1>
                            <p class="mt-4 text-sm leading-6 text-white/52">
                                Indiquez ce qui vous intéresse, vos projets, vos habitudes et ce que
                                vous ne souhaitez pas voir. Wasplex utilise uniquement les
                                informations autorisées pour sélectionner des publicités plus
                                pertinentes.
                            </p>
                        </div>
                        <div
                        ...
                        >
                            <p
                                class="text-[0.65rem] font-black tracking-widest text-white/35 uppercase"
                            >
                                Marché initial
                            </p>
                            <p class="mt-1 font-black text-wasplex-gold">
                                {{ profile.market.name }}
                            </p>
                            <p class="mt-1 text-[0.65rem] text-white/30">
                ...
                <div
                    v-if="flash.success"
                    class="mt-5 rounded-2xl border border-emerald-300/22 bg-emerald-300/8 px-4 py-3 text-sm font-bold text-emerald-200"
                >
                    {{ flash.success }}
                </div>
                ...
                <div
                    v-if="
                        errors.answer?.length ||
                        errors.status?.length ||
                        errors.purpose?.length ||
                        errors.question?.length
                    "
                ...
                <section class="mt-7 grid gap-3 sm:grid-cols-3">
                    <article class="rounded-2xl border border-white/8 bg-white/[0.035] p-5">
                        ...
                        <p class="mt-3 text-3xl font-black">
                            {{ profile.summary.activeInformation }}
                        </p>
                        <p class="mt-2 text-xs leading-5 text-white/32">
                            Déclarées ou confirmées par vous.
                        </p>
                    </article>
                    <article
                        class="rounded-2xl border border-wasplex-gold/15 bg-wasplex-gold/[0.045] p-5"
                    >
                        <p
                            class="text-xs font-black tracking-widest text-wasplex-gold/70 uppercase"
                        >
                            Secteurs explorés
                        </p>
                        <p class="mt-3 text-3xl font-black text-wasplex-gold">
                            {{ profile.summary.sectorsExplored }}
                        </p>
                        <p class="mt-2 text-xs leading-5 text-white/32">
                            Sur {{ profile.summary.availableSectors }} secteurs disponibles.
                        </p>
                    </article>
                    <article
                        class="rounded-2xl border border-wasplex-cyan/15 bg-wasplex-cyan/[0.045] p-5"
                    >
                        <p
                            class="text-xs font-black tracking-widest text-wasplex-cyan/70 uppercase"
                        >
                            Suggestions à confirmer
                        </p>
                        <p class="mt-3 text-3xl font-black text-wasplex-cyan">
                            {{ profile.summary.pendingSuggestions }}
                        </p>
                        <p class="mt-2 text-xs leading-5 text-white/32">
                            Une proposition IA n’est jamais utilisée sans confirmation.
                        </p>
                    </article>
                </section>
                ...
                <section class="mt-9">
                    <p class="text-xs font-black tracking-widest text-white/35 uppercase">
                        Carte de mes intérêts et projets
                    </p>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-2 text-xs text-white/32">
                                        {{
                                            sector.questions > 0
                                                ? `${sector.questions} question(s) disponible(s)`
                                                : 'Catalogue prêt à être enrichi'
                                        }}
                                    </p>
                                </div>
                                <span
                                    v-if="sector.explored"
                                    class="rounded-full bg-emerald-300/12 px-2 py-1 text-[0.6rem] font-black text-emerald-200"
                                    >Exploré</span
                                >
                            </div>
                        </article>
                    ...
                    <p class="mt-4 text-xs leading-5 text-white/30">
                        Aucun secteur n’est principal. L’administration Wasplex peut ajouter de
                        nouveaux secteurs, centres d’intérêt et questions sans reconstruire
                        l’application.
                    </p>
                </section>
                ...
                <section class="mt-10">
                    <p class="text-xs font-black tracking-widest text-white/35 uppercase">
                        Questions facultatives et progressives
                    </p>
                    <h2 class="mt-2 text-xl font-black">
                        Précisez seulement ce qui vous est utile
                    </h2>
                    <p class="mt-2 text-sm leading-6 text-white/42">
                        Un intérêt, une possession, une habitude et un projet sont traités comme des
                        informations différentes. Vous pouvez ignorer, modifier ou retirer chaque
                        réponse.
                    </p>
                                ...
                                >
                                    <div class="flex items-start justify-between gap-4">
                                        <div>
                                            ...
                                            >
                                                {{ signalLabel(question.signalKind) }} · Facultatif
                                            </p>
                                            <h4 class="mt-2 text-lg leading-6 font-black">
                                                {{ question.prompt }}
                                            </h4>
                                        </div>
                                        <span
                ...
                <section
                    class="mt-10 rounded-[2rem] border border-white/8 bg-white/[0.025] p-5 sm:p-7"
                >
                    <p class="text-xs font-black tracking-widest text-white/35 uppercase">
                        Mes autorisations
                    </p>
                    ...
                    <p class="mt-2 text-sm leading-6 text-white/42">
                        Les autorisations restent séparées, versionnées et retitables. Un retrait
                        bloque immédiatement les nouveaux matchings concernés.
                    </p>
                            ...
                            >
                                <div class="max-w-xl">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="rounded-full px-2 py-1 text-[0.6rem] font-black uppercase"
                                            :class="
                                                consent.active
                                    ...
                                    <p
                                        v-if="consent.refusalConsequence && !consent.active"
                                        class="mt-2 text-xs leading-5 text-white/30"
                                    >
                                        Sans autorisation : {{ consent.refusalConsequence }}
                                    </p>
                                </div>
                                <div class="flex shrink-0 gap-2">
                                    <button
                                        v-if="!consent.active"
                                        type="button"
                                        class="rounded-xl bg-emerald-300 px-4 py-2 text-xs font-black text-[#062018]"
                                    ...
                                    <button
                                        v-if="!consent.active"
                                        type="button"
                                        class="rounded-xl border border-white/10 px-4 py-2 text-xs font-black text-white/55"
                                    ...
                                    <button
                                        v-else
                                        type="button"
                                        class="rounded-xl border border-rose-300/20 px-4 py-2 text-xs font-black text-rose-200"
