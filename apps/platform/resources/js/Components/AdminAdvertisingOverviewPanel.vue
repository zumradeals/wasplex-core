    ...
    try {
        ...
        pendingAdvertisers.value =
            advertisersRes.status === 'fulfilled'
                ? (advertisersRes.value.data.advertisers as Advertiser[]).filter(
                      (advertiser) => advertiser.status === 'draft',
                  ).length
                : 0;
        activeCampaigns.value = campaignsRes.status === 'fulfilled' ? campaignsRes.value.data.campaigns.length : 0;
...
<template>
    <div class="text-wpx-white-soft flex flex-col gap-5">
        <section class="border-wpx-border-dark rounded-wpx-xl bg-wpx-navy-850 shadow-wpx-card-dark border p-5 md:p-6">
            <p class="text-wpx-cyan text-[11px] font-extrabold tracking-[0.16em] uppercase">Centre publicité</p>
        ...
        <template v-else>
            <section class="grid grid-cols-2 gap-3 lg:grid-cols-4">
                ...
                <button
                    ...
                    @click="emit('openTab', 'advertisers')"
                >
                    <p class="text-wpx-muted-dark text-[10px] font-extrabold tracking-wide uppercase">En diffusion</p>
                ...
                >
                    <p class="text-wpx-muted-dark text-[10px] font-extrabold tracking-wide uppercase">Antifraude</p>
                    <p class="text-wpx-danger mt-2 text-2xl font-extrabold">{{ holds.length }}</p>
                    <p class="text-wpx-muted-dark mt-1 text-[11px]">
                        {{ numberFormatter.format(heldAmount) }} WP retenus
                    </p>
                </button>
            </section>
            ...
            <section class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                <div class="border-wpx-border-dark rounded-wpx-xl bg-wpx-navy-850 shadow-wpx-card-dark border p-5">
                    ...
                    <div class="mt-5 grid grid-cols-2 gap-3">
                        ...
                        <div class="border-wpx-border-dark rounded-wpx-lg bg-wpx-navy-950 border p-4">
                            ...
                            <p class="text-wpx-gold mt-1 text-xl font-extrabold">
                                {{ numberFormatter.format(wpDistributed) }} WP
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-wpx-border-dark rounded-wpx-xl bg-wpx-navy-850 shadow-wpx-card-dark border p-5">
                    ...
                    <p class="text-wpx-muted-dark mt-1 text-xs">
                        Ces réglages servent à protéger les utilisateurs et à définir quelles informations volontaires
                        peuvent participer au ciblage.
                    </p>
                    <div class="mt-4 flex flex-wrap gap-2">
                        <button
                            type="button"
                            class="border-wpx-border-dark rounded-wpx-full text-wpx-cyan border px-3 py-2 text-xs font-bold"
