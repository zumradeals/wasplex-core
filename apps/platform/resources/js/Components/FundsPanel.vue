...
const wish = ref({ category_id: '', title: '', description: '', city: '', estimated_amount_minor: null as number | null });
...
async function join(): Promise<void> {
    ...
    try {
        ...
        error.value = e?.response?.data?.message ?? 'L’adhésion n’a pas pu être enregistrée.';
    } finally {
        busy.value = false;
    }
}
...
async function createWish(): Promise<void> {
    ...
    try {
        ...
        error.value = e?.response?.data?.message ?? 'Le brouillon n’a pas pu être créé.';
    } finally {
        busy.value = false;
    }
}
...
async function submitWish(item: Wish): Promise<void> {
    ...
    try {
        ...
        error.value = e?.response?.data?.message ?? 'Le vœu n’a pas pu être soumis.';
    } finally {
        busy.value = false;
    }
}
...
<template>
    <div class="flex flex-col gap-4 pb-4">
        <section class="from-wpx-navy-750 via-wpx-navy-850 to-wpx-navy-950 border-wpx-border-dark rounded-wpx-xl relative overflow-hidden border bg-gradient-to-br p-5">
            <div class="bg-wpx-cyan/8 absolute -top-12 -right-12 h-36 w-36 rounded-full blur-2xl"></div>
            <div class="relative">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-wpx-cyan text-[11px] font-bold tracking-[0.18em] uppercase">Solidarité Wasplex</p>
                        ...
                        <p class="text-wpx-muted-dark mt-2 max-w-xs text-sm leading-relaxed">
                            Construis ton projet avec ton apport, puis avance avec la force d’une communauté organisée et vérifiée.
                        </p>
                    </div>
                    <span class="from-wpx-blue/20 to-wpx-cyan/10 border-wpx-cyan/20 flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border bg-gradient-to-br">
        ...
        <template v-if="!loading && data">
            <section v-if="activeMembership" class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-xl border p-4">
                <div class="flex items-start justify-between gap-3">
                    ...
                    <span class="rounded-full px-2.5 py-1 text-[11px] font-bold" :class="statusClass(activeMembership.status)">
                ...
                <div class="mt-4 grid grid-cols-2 gap-2.5">
                    <div class="bg-wpx-navy-950 rounded-wpx-lg p-3">
                        ...
                        <p class="text-wpx-white-soft mt-1 text-sm font-bold">{{ money(activeMembership.mandate?.personal_cap_minor ?? null) }}</p>
                    </div>
                    <div class="bg-wpx-navy-950 rounded-wpx-lg p-3">
                        <p class="text-wpx-muted-dark text-[10px] uppercase">Mandat</p>
                        <p class="mt-1 text-sm font-bold" :class="activeMembership.mandate?.is_active ? 'text-wpx-success-light' : 'text-wpx-danger'">
                ...
                <p v-if="activeMembership.status === 'grace_period'" class="text-wpx-gold mt-3 text-xs leading-relaxed">
                    Votre abonnement Wasplex doit être renouvelé. Aucun nouveau vœu ni nouvel engagement Fonds n’est créé pendant ce délai.
                </p>
                <button
            ...
            <section v-else-if="!data.eligible" class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-xl border p-5 text-center">
            ...
            <section v-else-if="data.programs.length === 0" class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-xl border p-5 text-center">
                <p class="text-wpx-white-soft font-bold">Les programmes arrivent bientôt</p>
                <p class="text-wpx-muted-dark mt-1 text-sm">Aucun programme Fonds n’est actuellement ouvert aux nouvelles adhésions.</p>
            </section>
            ...
            <section v-else-if="!activeMembership" class="flex flex-col gap-3">
                <div>
                    ...
                    <p class="text-wpx-muted-dark mt-1 text-xs">Les règles et plafonds restent visibles avant toute adhésion.</p>
                </div>
                <button
                ...
                >
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="text-wpx-muted-dark mt-0.5 text-xs">{{ program.max_wishes_per_period }} vœu(x) · apport {{ program.personal_contribution_percent }}%</p>
                        </div>
                    ...
                    <div class="mt-3 grid grid-cols-3 gap-2">
                        <div class="bg-wpx-navy-950 rounded-lg p-2.5">
                            ...
                            <p class="text-wpx-white-soft mt-1 text-xs font-bold">{{ money(program.min_debit_minor) }} min.</p>
                        </div>
                        <div class="bg-wpx-navy-950 rounded-lg p-2.5">
            ...
            <section v-if="wishes.length" class="flex flex-col gap-2.5">
                ...
                <article v-for="item in wishes" :key="item.id" class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-lg border p-3.5">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="text-wpx-muted-dark text-[10px] font-bold uppercase">{{ item.category?.icon }} {{ item.category?.name }}</p>
                            <h3 class="text-wpx-white-soft mt-1 truncate text-sm font-bold">{{ item.title }}</h3>
                        </div>
                        <span class="shrink-0 rounded-full px-2 py-1 text-[10px] font-bold" :class="statusClass(item.status)">{{ statusLabel(item.status) }}</span>
                    </div>
                    <div v-if="item.estimated_amount_minor" class="mt-3 flex items-center justify-between border-t border-white/5 pt-3 text-xs">
                        <span class="text-wpx-muted-dark">Budget estimé</span>
                        <span class="text-wpx-white-soft font-bold">{{ money(item.estimated_amount_minor, item.currency) }}</span>
                    </div>
        ...
        <Teleport to="body">
            <div v-if="showJoin && selectedProgram" class="fixed inset-0 z-50 flex items-end justify-center bg-black/70 sm:items-center" @click.self="showJoin = false">
                <div class="bg-wpx-navy-950 border-wpx-border-dark max-h-[90vh] w-full max-w-md overflow-y-auto rounded-t-3xl border p-5 sm:rounded-3xl">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <button class="bg-wpx-navy-750 text-wpx-white-soft h-9 w-9 rounded-full" @click="showJoin = false">×</button>
                    </div>
                    <div class="bg-wpx-navy-850 mt-4 rounded-xl p-4">
                        ...
                        <p class="text-wpx-muted-dark mt-2 text-xs leading-relaxed">
                            Wasplex pourra débiter automatiquement votre Solde Fonds lorsqu’une collecte éligible est programmée, sans confirmation à chaque vœu, dans la limite du plafond choisi. Vous serez informé avant le débit.
                        </p>
                    </div>
                    <label class="text-wpx-muted-dark mt-4 block text-xs font-semibold">Mon plafond par contribution</label>
                    <input v-model.number="personalCap" type="number" :min="selectedProgram.min_debit_minor" :max="selectedProgram.max_debit_minor ?? undefined" class="bg-wpx-navy-850 border-wpx-border-dark text-wpx-white-soft mt-1 w-full rounded-xl border px-3 py-3" />
                    <p class="text-wpx-muted-dark mt-1 text-[11px]">Minimum {{ money(selectedProgram.min_debit_minor) }}<template v-if="selectedProgram.max_debit_minor"> · maximum {{ money(selectedProgram.max_debit_minor) }}</template></p>
                    <label class="mt-4 flex items-start gap-3">
                        <input v-model="acceptMandate" type="checkbox" class="mt-1" />
                        <span class="text-wpx-muted-dark text-xs leading-relaxed">J’accepte le mandat de contributions automatiques plafonnées et les règles de ce programme.</span>
                    </label>
                    <button type="button" class="from-wpx-orange to-wpx-gold text-wpx-navy-950 mt-5 w-full rounded-xl bg-gradient-to-r px-4 py-3 font-extrabold disabled:opacity-40" :disabled="busy || !acceptMandate" @click="join">Confirmer mon adhésion</button>
                </div>
            </div>

            <div v-if="showWish" class="fixed inset-0 z-50 flex items-end justify-center bg-black/70 sm:items-center" @click.self="showWish = false">
                <div class="bg-wpx-navy-950 border-wpx-border-dark max-h-[92vh] w-full max-w-md overflow-y-auto rounded-t-3xl border p-5 sm:rounded-3xl">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                        ...
                        <button class="bg-wpx-navy-750 text-wpx-white-soft h-9 w-9 rounded-full" @click="showWish = false">×</button>
                    </div>
                    <p class="text-wpx-muted-dark mt-2 text-xs leading-relaxed">Commence par l’essentiel. Le dossier pourra être complété et vérifié avant toute collecte.</p>
                    <label class="text-wpx-muted-dark mt-4 block text-xs font-semibold">Catégorie</label>
                    <select v-model="wish.category_id" class="bg-wpx-navy-850 border-wpx-border-dark text-wpx-white-soft mt-1 w-full rounded-xl border px-3 py-3">
                        <option value="" disabled>Choisir une catégorie</option>
                        <option v-for="category in data?.categories ?? []" :key="category.id" :value="category.id">{{ category.icon }} {{ category.name }}</option>
                    </select>
                    <label class="text-wpx-muted-dark mt-3 block text-xs font-semibold">Titre</label>
                    <input v-model="wish.title" maxlength="120" placeholder="Ex. Une moto pour mon activité" class="bg-wpx-navy-850 border-wpx-border-dark text-wpx-white-soft mt-1 w-full rounded-xl border px-3 py-3" />
                    <label class="text-wpx-muted-dark mt-3 block text-xs font-semibold">Explique ton besoin</label>
                    <textarea v-model="wish.description" rows="4" placeholder="Décris simplement le besoin, son utilité et ta situation." class="bg-wpx-navy-850 border-wpx-border-dark text-wpx-white-soft mt-1 w-full rounded-xl border px-3 py-3"></textarea>
                    <div class="mt-3 grid grid-cols-2 gap-2.5">
                        <div>
                            <label class="text-wpx-muted-dark block text-xs font-semibold">Ville</label>
                            <input v-model="wish.city" placeholder="Abidjan" class="bg-wpx-navy-850 border-wpx-border-dark text-wpx-white-soft mt-1 w-full rounded-xl border px-3 py-3" />
                        </div>
                        <div>
                            <label class="text-wpx-muted-dark block text-xs font-semibold">Budget estimé</label>
                            <input v-model.number="wish.estimated_amount_minor" type="number" min="1" placeholder="FCFA" class="bg-wpx-navy-850 border-wpx-border-dark text-wpx-white-soft mt-1 w-full rounded-xl border px-3 py-3" />
                        </div>
                    </div>
                    <button type="button" class="from-wpx-blue to-wpx-cyan text-wpx-navy-950 mt-5 w-full rounded-xl bg-gradient-to-r px-4 py-3 font-extrabold disabled:opacity-40" :disabled="busy || !wish.category_id || wish.title.length < 2 || wish.description.length < 20" @click="createWish">Enregistrer mon brouillon</button>
                </div>
            </div>
        </Teleport>
    </div>
