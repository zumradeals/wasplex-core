...
<template>
    <section class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-xl overflow-hidden border">
        <div class="from-wpx-blue/12 to-wpx-gold/8 bg-gradient-to-br via-transparent p-4">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-wpx-cyan text-[10px] font-extrabold tracking-[0.16em] uppercase">Mon Solde Fonds</p>
                    <div class="mt-1 flex items-baseline gap-2">
                        <strong class="text-wpx-white-soft text-2xl font-black">{{
                            format(balances.fund_balance_minor)
                        }}</strong>
                        <span class="text-wpx-gold text-xs font-extrabold">WP</span>
                    </div>
                    <p class="text-wpx-muted-dark mt-1 text-[11px]">
                        ≈ {{ format(balances.fund_balance_minor) }} FCFA · 1 WP = 1 FCFA
                    </p>
                </div>
                <div class="bg-wpx-navy-950/80 border-wpx-border-dark rounded-xl border px-3 py-2 text-right">
                    ...
                    <p class="text-wpx-white-soft mt-0.5 text-xs font-bold">
                        {{ format(balances.wallet_available_minor) }} WP
                    </p>
                </div>
            </div>

            <p class="text-wpx-muted-dark mt-3 text-xs leading-relaxed">
                Ce solde est réservé à Fonds. Tu gardes une vue claire de ce qui reste disponible dans ton Wallet et de
                ce qui est préparé pour tes engagements solidaires.
            </p>
