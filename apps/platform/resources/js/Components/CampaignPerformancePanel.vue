        ...
        <div v-if="!loading && publicReport" class="border-wpx-border-dark bg-wpx-navy-950 rounded-wpx-lg border p-4">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <h3 class="text-wpx-white-soft mt-1 text-sm font-extrabold">Diffuser aussi hors connexion Wasplex</h3>
                    <p class="text-wpx-muted-dark mt-1 max-w-2xl text-xs leading-relaxed">
                        Une vue publique apporte de la visibilité supplémentaire, mais verse 0 WP, ne consomme aucun quota membre et ne touche jamais au budget récompense.
                    </p>
                </div>
                <span
                    class="rounded-full px-2.5 py-1 text-[10px] font-extrabold"
                    :class="publicReport.distribution_mode === 'members_public' ? 'bg-wpx-success/12 text-wpx-success-light' : 'bg-wpx-navy-750 text-wpx-muted-dark'"
                >
            ...
            <div class="mt-4 grid gap-2 sm:grid-cols-2">
                <button
                    ...
                    :class="publicReport.distribution_mode === 'members_only' ? 'border-wpx-gold bg-wpx-gold/10 text-wpx-gold' : 'border-wpx-border-dark bg-wpx-navy-850 text-wpx-muted-dark'"
                ...
                <button
                    type="button"
                    class="rounded-xl border p-3 text-left text-xs font-bold disabled:opacity-50"
                    :class="publicReport.distribution_mode === 'members_public' ? 'border-wpx-cyan bg-wpx-cyan/10 text-wpx-cyan' : 'border-wpx-border-dark bg-wpx-navy-850 text-wpx-muted-dark'"
                ...
                >
                    ...
                    <span class="mt-1 block font-normal opacity-75">
                        {{ publicReport.status === 'approved' ? 'Lien public partageable, sans rémunération.' : 'Disponible après approbation.' }}
                    </span>
                </button>
            </div>

            <div v-if="publicReport.distribution_mode === 'members_public' && publicReport.public_url" class="border-wpx-border-dark bg-wpx-navy-850 mt-4 rounded-xl border p-3">
                <p class="text-wpx-muted-dark text-[10px] font-bold uppercase">Lien public</p>
                <p class="text-wpx-white-soft mt-1 break-all text-xs font-semibold">{{ publicReport.public_url }}</p>
                <div class="mt-3 flex flex-wrap gap-2">
                    <button type="button" class="bg-wpx-gold text-wpx-navy-950 rounded-lg px-3 py-2 text-[11px] font-extrabold" @click="copyPublicLink">
                        {{ copied ? 'Copié ✓' : 'Copier le lien' }}
                    </button>
                    <a :href="publicReport.public_url" target="_blank" rel="noopener" class="border-wpx-border-dark text-wpx-cyan rounded-lg border px-3 py-2 text-[11px] font-bold">
                        Ouvrir la page ↗
                    </a>
                </div>
            </div>

            <div v-if="publicReport.distribution_mode === 'members_public'" class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
                <div class="bg-wpx-navy-850 rounded-xl p-3">
                    <p class="text-wpx-muted-dark text-[10px]">Vues publiques</p>
                    <p class="text-wpx-white-soft mt-1 text-lg font-black">{{ numberFormatter.format(publicReport.stats.views) }}</p>
                </div>
                <div class="bg-wpx-navy-850 rounded-xl p-3">
                    <p class="text-wpx-muted-dark text-[10px]">Visiteurs approx.</p>
                    <p class="text-wpx-white-soft mt-1 text-lg font-black">{{ numberFormatter.format(publicReport.stats.unique_visitors) }}</p>
                </div>
                <div class="bg-wpx-navy-850 rounded-xl p-3">
                    <p class="text-wpx-muted-dark text-[10px]">Lectures complètes</p>
                    <p class="text-wpx-white-soft mt-1 text-lg font-black">{{ numberFormatter.format(publicReport.stats.completions) }}</p>
                </div>
                <div class="bg-wpx-navy-850 rounded-xl p-3">
                    <p class="text-wpx-muted-dark text-[10px]">Partages</p>
                    <p class="text-wpx-white-soft mt-1 text-lg font-black">{{ numberFormatter.format(publicReport.stats.shares) }}</p>
                </div>
                <div class="bg-wpx-navy-850 rounded-xl p-3">
                    <p class="text-wpx-muted-dark text-[10px]">Vers Wasplex</p>
                    <p class="text-wpx-white-soft mt-1 text-lg font-black">{{ numberFormatter.format(publicReport.stats.wasplex_cta_clicks) }}</p>
                </div>
            </div>
        </div>
    </div>
