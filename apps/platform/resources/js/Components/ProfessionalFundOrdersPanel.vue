    ...
    <section class="mt-4 flex flex-col gap-4">
        <div class="border-wpx-border-dark bg-wpx-navy-850 rounded-3xl border p-5">
            ...
            <p class="text-wpx-muted-dark mt-2 text-[11px] leading-relaxed">
                Une commande n’apparaît ici qu’après sélection de votre devis et financement effectif. Le règlement
                externe reste distinct de l’allocation Ledger Wasplex.
            </p>
        </div>
        ...
        <div
            ...
            class="border-wpx-border-dark bg-wpx-navy-850 text-wpx-muted-dark rounded-3xl border p-6 text-center text-xs"
        >
        ...
        >
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p class="text-wpx-muted-dark mt-1 text-[10px]">
                        {{ order.wish.category?.icon }} {{ order.wish.category?.name }}
                    </p>
                </div>
                <div class="text-right">
                    ...
                    <p class="text-wpx-muted-dark mt-1 text-[9px]">
                        Règlement confirmé : {{ money(order.externally_settled_minor, order.currency) }}
                    </p>
                </div>
            </div>
            <div class="mt-4 space-y-2">
                <div
                    ...
                    :key="milestone.id"
                    class="border-wpx-border-dark bg-wpx-navy-950/60 flex flex-wrap items-center justify-between gap-2 rounded-2xl border p-3"
                >
                    <div>
                        ...
                        <p class="text-wpx-muted-dark mt-1 text-[9px]">
                            {{ milestone.status }} ·
                            {{
                                milestone.external_settlement_status === 'confirmed'
                                    ? 'règlement externe confirmé'
                                    : 'règlement à confirmer'
                            }}
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <button
                            v-if="props.canAct && ['pending', 'proof_submitted'].includes(milestone.status)"
                            type="button"
                            class="bg-wpx-cyan/10 text-wpx-cyan rounded-xl px-3 py-2 text-[9px] font-black"
                            @click="openProof(order, milestone)"
                        >
                            Preuve
                        </button>
                    </div>
            ...
            <button
                ...
                type="button"
                class="bg-wpx-success/15 text-wpx-success-light mt-4 w-full rounded-2xl px-4 py-3 text-[10px] font-black"
                @click="deliveryOrder = order"
            >
        ...
        >
            <div class="bg-wpx-navy-950 border-wpx-border-dark w-full max-w-md rounded-t-3xl border p-5 sm:rounded-3xl">
                ...
                <input
                    ...
                    placeholder="Référence facture / BL / dossier"
                ...
                />
                <button
                    class="bg-wpx-cyan text-wpx-navy-950 mt-3 w-full rounded-xl px-4 py-3 text-sm font-black"
                    :disabled="busy"
                    @click="submitProof"
                >
        ...
        >
            <div class="bg-wpx-navy-950 border-wpx-border-dark w-full max-w-md rounded-t-3xl border p-5 sm:rounded-3xl">
                ...
                <button
                    ...
                    :disabled="busy || !deliveryReference.trim()"
                    @click="markDelivered"
                >
