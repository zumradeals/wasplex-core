                ...
                <template v-else-if="card">
                    <section class="from-wpx-navy-700 via-wpx-navy-850 to-wpx-navy-950 border-wpx-gold/25 relative overflow-hidden rounded-[26px] border bg-gradient-to-br p-5 shadow-2xl">
                        ...
                        <div class="relative mt-10">
                            ...
                            <p class="text-wpx-gold mt-4 font-mono text-sm tracking-[0.08em] break-all">{{ card.public_identifier }}</p>
                        </div>
                    ...
                    <section class="mt-4 grid grid-cols-2 gap-2.5">
                        <button type="button" :disabled="busy || card.status !== 'active'" class="from-wpx-orange to-wpx-gold text-wpx-navy-950 rounded-xl bg-gradient-to-r px-3 py-3 text-sm font-extrabold disabled:opacity-40" @click="paymentsOpen = true">Recevoir / Payer</button>
                        <button type="button" :disabled="busy || card.status !== 'active'" class="border-wpx-gold/25 bg-wpx-navy-850 text-wpx-gold rounded-xl border px-3 py-3 text-sm font-bold disabled:opacity-40" @click="generateQr">QR identité</button>
                        <button type="button" class="border-wpx-border-dark bg-wpx-navy-850 text-wpx-white-soft col-span-2 rounded-xl border px-3 py-3 text-sm font-bold" @click="emit('close')">Fermer</button>
                    </section>
                    ...
                    <section class="border-wpx-border-dark bg-wpx-navy-850 mt-4 rounded-2xl border p-4">
                        ...
                        <p class="text-wpx-muted-dark mt-1.5 text-xs leading-relaxed">Votre QR ne révèle ni téléphone, ni e-mail, ni solde Wallet, ni KYC, ni données Santé ou Fonds. Le QR d’identité expire après 2 minutes. Un QR de réception ne déclenche aucun débit au simple scan.</p>
                    </section>
