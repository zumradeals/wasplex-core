...
function choose(alert: AdminAlert): void {
    ...
    publicSummary.value = alert.public_summary || alert.description;
...
async function reject(): Promise<void> {
    ...
    if (rejectionReason.value.trim().length < 5) {
        error.value = 'Expliquez en quelques mots ce que le citoyen doit corriger.';
        return;
    }
    ...
    try {
        ...
        notice.value = 'Alerte refusée avec un motif transmis au citoyen.';
        selected.value = null;
        rejectionReason.value = '';
        await load();
    } catch {
...
<template>
    <div class="flex flex-col gap-5">
        <section
            class="from-wpx-navy-750 via-wpx-navy-850 to-wpx-navy-950 border-wpx-border-dark rounded-wpx-xl border bg-gradient-to-br p-5 shadow-xl"
        >
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <p class="text-wpx-cyan text-[10px] font-black tracking-[0.16em] uppercase">
                        Protection & communauté
                    </p>
        ...
        <p
            v-if="notice"
            class="text-wpx-success-light rounded-wpx-lg bg-emerald-500/10 px-4 py-3 text-xs font-semibold"
        >
            {{ notice }}
        </p>
        <p v-if="error" class="text-wpx-danger rounded-wpx-lg bg-red-500/10 px-4 py-3 text-xs font-semibold">
            {{ error }}
        </p>
        ...
        <section class="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
            <div class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-xl border p-4">
                ...
                <div
                    v-else-if="submittedAlerts.length === 0"
                    class="text-wpx-muted-dark rounded-2xl bg-black/10 p-5 text-center text-xs"
                >
                    ...
                    <button
                        ...
                        :class="
                            selected?.id === alert.id
                                ? 'bg-wpx-cyan/8 border-wpx-cyan/40'
                                : 'bg-black/10 hover:bg-white/4'
                        "
                        @click="choose(alert)"
                    >
            ...
            <div class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-xl border p-4">
                <template v-if="selected">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <p class="text-wpx-gold text-[9px] font-black">
                                {{ selected.priority }} · {{ categoryLabel(selected.category) }}
                            </p>
                    ...
                    <div class="border-wpx-border-dark mt-4 rounded-2xl border bg-black/10 p-3">
                        ...
                        <p class="text-wpx-white-soft mt-1 text-xs">
                            {{ selected.area_label || selected.city || 'Non précisée' }}
                        </p>
                        <p class="text-wpx-muted-dark mt-2 text-[9px]">
                            La position exacte privée n’est pas exposée dans cet écran.
                        </p>
                    </div>
                    ...
                    <select
                        ...
                        class="bg-wpx-navy-950 border-wpx-border-dark text-wpx-white-soft mt-1 w-full rounded-xl border px-3 py-2.5 text-xs"
                    >
                    ...
                    <label class="text-wpx-muted-dark mt-3 block text-[10px] font-bold">Motif si vous refusez</label>
                    <textarea
                        v-model="rejectionReason"
                        rows="3"
                        maxlength="1000"
                        class="bg-wpx-navy-950 border-wpx-border-dark text-wpx-white-soft placeholder:text-wpx-muted-dark mt-1 w-full rounded-2xl border px-3 py-3 text-xs leading-relaxed"
                        placeholder="Ex. Précisez le quartier, sans publier l’adresse exacte."
                    />
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-2">
                        <button
                            type="button"
                            class="border-wpx-danger/40 text-wpx-danger rounded-xl border py-3 text-xs font-black"
                            :disabled="saving"
                            @click="reject"
                        >
                            Refuser
                        ...
                        <button
                            type="button"
                            class="from-wpx-orange to-wpx-gold text-wpx-navy-950 rounded-xl bg-gradient-to-r py-3 text-xs font-black"
                            :disabled="saving"
                            @click="publish"
                        >
                            Publier
                ...
                <div
                    v-else
                    class="text-wpx-muted-dark flex min-h-72 flex-col items-center justify-center text-center text-xs"
                >
        ...
        <section class="bg-wpx-navy-850 border-wpx-border-dark rounded-wpx-xl border p-5">
            ...
            <div class="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <label class="bg-wpx-navy-950 border-wpx-border-dark rounded-2xl border p-3">
                    ...
                    <select
                        v-model="usefulContentInterval"
                        class="border-wpx-border-dark text-wpx-white-soft mt-2 w-full rounded-xl border bg-black/20 px-3 py-2 text-xs"
                    >
                ...
                <label
                    class="bg-wpx-navy-950 border-wpx-border-dark flex items-center justify-between rounded-2xl border p-3"
                >
                    <span
                        ><span class="text-wpx-white-soft block text-xs font-black">Cercles</span
                        ><span class="text-wpx-muted-dark text-[9px]">En haut du Feed</span></span
                    >
                ...
                >
                    <span
                        ><span class="text-wpx-white-soft block text-xs font-black">Rail gauche</span
                        ><span class="text-wpx-muted-dark text-[9px]">Face aux actions sociales</span></span
                    >
                ...
                >
                    <span
                        ><span class="text-wpx-white-soft block text-xs font-black">Plein écran</span
                        ><span class="text-wpx-muted-dark text-[9px]">Contenu utile entre pubs</span></span
                    >
            ...
            <div class="border-wpx-border-dark mt-5 border-t pt-5">
                ...
                <div class="grid gap-2 lg:grid-cols-2">
                    <div
                        ...
                        :key="index"
                        class="bg-wpx-navy-950 border-wpx-border-dark rounded-2xl border p-3"
                    >
                        <div class="flex gap-2">
                            <input
                                v-model="tip.title"
                        ...
                        <textarea
                            ...
                            maxlength="500"
                            class="border-wpx-border-dark text-wpx-white-soft mt-2 w-full rounded-xl border bg-black/20 px-3 py-2 text-xs"
                            placeholder="Conseil simple et utile"
                        />
                    </div>
                </div>
            </div>
            ...
            <div class="mt-5 flex justify-end">
                <button
                    type="button"
                    class="from-wpx-blue to-wpx-cyan text-wpx-navy-950 rounded-xl bg-gradient-to-r px-5 py-3 text-xs font-black"
                    :disabled="saving"
                    @click="saveSettings"
                >
