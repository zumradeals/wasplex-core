...
async function resolve(hold: Hold, action: 'release' | 'reject'): Promise<void> {
    const promptText =
        action === 'release' ? 'Note de vérification avant versement (optionnelle) :' : 'Motif du refus (optionnel) :';
...
<template>
    <section class="border-wpx-border-dark rounded-wpx-xl bg-wpx-navy-850 shadow-wpx-card-dark overflow-hidden border">
        <div
            class="border-wpx-border-dark flex flex-col gap-3 border-b px-5 py-4 sm:flex-row sm:items-center sm:justify-between"
        >
            <div>
                ...
                <p class="text-wpx-muted-dark mt-1 text-xs">
                    Santé économique, retenues antifraude et décisions humaines.
                </p>
            </div>
            <button
                type="button"
                class="border-wpx-border-dark text-wpx-white-soft rounded-wpx-md border px-3 py-2 text-xs font-bold"
                @click="load"
            >
                Actualiser
            </button>
            ...
            <div class="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="border-wpx-border-dark bg-wpx-navy-950 rounded-xl border p-3">
                    ...
                    <p
                        ...
                        :class="dashboard.rewards_enabled ? 'text-emerald-300' : 'text-wpx-danger'"
                    >
                        {{ dashboard.rewards_enabled ? 'Actives' : 'Suspendues' }}
                    </p>
                ...
                <div class="border-wpx-border-dark bg-wpx-navy-950 rounded-xl border p-3">
                    ...
                    <p class="text-wpx-white-soft mt-1 text-lg font-extrabold">
                        {{ dashboard.active_sponsored_lives }}
                    </p>
                    <p class="text-wpx-muted-dark text-[10px]">
                        {{ dashboard.active_rewarded_seats }} places rémunérées actives
                    </p>
                </div>
                <div class="border-wpx-border-dark bg-wpx-navy-950 rounded-xl border p-3">
                    ...
                    <p class="text-wpx-muted-dark text-[10px]">
                        membres · Wasplex {{ money(dashboard.wasplex_revenue_minor) }}
                    </p>
                </div>
                <div class="border-wpx-border-dark bg-wpx-navy-950 rounded-xl border p-3">
                    ...
                    <p
                        ...
                        :class="dashboard.pending_holds ? 'text-orange-300' : 'text-emerald-300'"
                    >
                        {{ dashboard.pending_holds }}
                    </p>
                    <p class="text-wpx-muted-dark text-[10px]">
                        {{ money(dashboard.pending_review_minor) }} membres en attente
                    </p>
                </div>
            </div>

            <div class="border-wpx-border-dark grid gap-2 border-y p-4 text-xs sm:grid-cols-3">
                <p class="text-wpx-muted-dark">
                    Blocs : <strong class="text-wpx-white-soft">{{ dashboard.captured_blocks }} payés</strong> ·
                    {{ dashboard.held_blocks }} retenus · {{ dashboard.rejected_blocks }} refusés
                </p>
                <p class="text-wpx-muted-dark">
                    Signaux 24 h : <strong class="text-wpx-white-soft">{{ dashboard.risk_events_24h }}</strong> ·
                    {{ dashboard.critical_risk_events_24h }} critiques
                </p>
                <p class="text-wpx-muted-dark">
                    Brut en revue :
                    <strong class="text-wpx-white-soft">{{ money(dashboard.pending_review_gross_minor) }}</strong>
                </p>
            </div>
            ...
            <div class="p-4">
                <div class="mb-3 flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-wpx-muted-dark mt-1 text-[11px]">
                            Le membre n’est pas payé tant que la décision n’est pas prise.
                        </p>
                    </div>
                    <span
                        class="rounded-full px-3 py-1 text-xs font-bold"
                        :class="
                            holds.length ? 'bg-orange-400/15 text-orange-200' : 'bg-emerald-400/15 text-emerald-200'
                        "
                        >{{ holds.length ? `${holds.length} dossier(s)` : 'Rien à examiner' }}</span
                    >
                </div>
                ...
                <div v-if="holds.length" class="space-y-3">
                    <article
                        v-for="hold in holds"
                        :key="hold.id"
                        class="border-wpx-border-dark bg-wpx-navy-950 rounded-xl border p-4"
                    >
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                            <div class="min-w-0">
                                <p class="text-wpx-white-soft text-sm font-extrabold">
                                    {{ hold.live_title ?? 'Live rémunéré' }}
                                </p>
                                <p class="text-wpx-muted-dark mt-1 text-[11px]">
                                    Bloc {{ hold.block_index }} · {{ money(hold.reward_minor) }} membre ·
                                    {{ money(hold.gross_amount_minor) }} brut réservé
                                </p>
                                <p class="mt-2 text-xs font-bold text-orange-200">
                                    {{ reasonLabels[hold.risk_codes?.[0]] ?? 'Contrôle de sécurité requis' }}
                                </p>
                            </div>
                            <div class="flex shrink-0 gap-2">
                                <button
                                    type="button"
                                    class="rounded-lg bg-emerald-400 px-3 py-2 text-xs font-extrabold text-slate-950 disabled:opacity-50"
                                    :disabled="busy === hold.id"
                                    @click="resolve(hold, 'release')"
                                >
                                    Verser
                                </button>
                                <button
                                    type="button"
                                    class="border-wpx-danger/50 text-wpx-danger rounded-lg border px-3 py-2 text-xs font-extrabold disabled:opacity-50"
                                    :disabled="busy === hold.id"
                                    @click="resolve(hold, 'reject')"
                                >
                                    Refuser
                                </button>
                            </div>
                        ...
                        <details
                            class="border-wpx-border-dark text-wpx-muted-dark mt-3 rounded-lg border p-3 text-[10px]"
                        >
                            <summary class="cursor-pointer font-bold">
                                Détails techniques réservés à l’administration
                            </summary>
                ...
                <p v-else class="text-wpx-muted-dark py-6 text-center text-xs">
                    Aucun gain Live n’est actuellement retenu.
                </p>
            </div>
        </template>
    </section>
