    ...
    <section v-if="loading || orders.length" class="flex flex-col gap-3">
        <div>
            ...
            <p class="text-wpx-muted-dark mt-1 text-xs">
                Commande, preuves, livraison et règlement sont suivis séparément.
            </p>
        </div>
        ...
        <article
            ...
            :key="order.id"
            class="bg-wpx-navy-850 border-wpx-border-dark rounded-2xl border p-4"
        >
            ...
            <div v-if="order.milestones.length" class="mt-3 space-y-2">
                <div
                    ...
                    :key="milestone.id"
                    class="border-wpx-border-dark flex items-center justify-between rounded-xl border px-3 py-2"
                >
                    <div>
            ...
            <div
                v-if="order.status === 'delivered' && !order.beneficiary_confirmed_at"
                class="mt-3 grid grid-cols-2 gap-2"
            >
                <button
                    class="bg-wpx-success/15 text-wpx-success-light rounded-xl px-3 py-2.5 text-[10px] font-black"
                    :disabled="busy"
                    @click="confirm(order)"
                >
                ...
                <button
                    class="bg-wpx-danger/10 text-wpx-danger rounded-xl px-3 py-2.5 text-[10px] font-black"
                    :disabled="busy"
                    @click="disputeOrder = order"
                >
    ...
    <Teleport to="body">
        ...
        >
            <div class="bg-wpx-navy-950 border-wpx-border-dark w-full max-w-md rounded-t-3xl border p-5 sm:rounded-3xl">
                ...
                <p class="text-wpx-muted-dark mt-1 text-xs">
                    Explique précisément ce qui ne correspond pas à la livraison attendue.
                </p>
                <textarea
                ...
                />
                <button
                    class="bg-wpx-danger mt-3 w-full rounded-xl px-4 py-3 text-sm font-black text-white disabled:opacity-40"
