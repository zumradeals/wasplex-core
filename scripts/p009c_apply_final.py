...
Path(
    "apps/platform/app/Modules/Distribution/Application/Services/FeedInteractionService.php"
).write_text(
    """<?php
...
once(
    "apps/platform/app/Modules/Distribution/Application/Services/AdvertisingDeliveryService.php",
...
once(
    "apps/platform/tests/Feature/Modules/Distribution/AdvertisingDeliveryTest.php",
...
once(
    "apps/platform/tests/Feature/Modules/Distribution/UserFeedPageTest.php",
...
feed = Path("apps/platform/resources/js/pages/User/Feed.vue")
text = feed.read_text()
replacements = [
    (
    ...
    (
        ...
        "'Le média met trop de temps à répondre.',",
    ),
    (
...
for old, new in replacements:
    if old not in text:
        raise SystemExit(f"Missing Feed anchor: {old[:80]!r}")
    text = text.replace(old, new, 1)

quota_pattern = re.compile(
    r'(?P<i>\s*)<p class="mt-3 text-sm font-black">\s*Quota restant : \{\{ delivery\.quota_remaining \}\}\s*</p>\s*<p class="mt-1 text-xs text-white/48">\s*La preuve d’attention sera traitée dans la phase suivante\.\s*</p>',
    re.MULTILINE,
)
quota_replacement = """
                            <p class="mt-3 text-xs font-black tracking-[0.16em] text-wasplex-cyan uppercase">
                                {{ delivery.brand_name || delivery.campaign_name || 'Annonceur Wasplex' }}
                            </p>
                            <h2 v-if="delivery.headline" class="mt-2 text-lg font-black leading-tight">
                                {{ delivery.headline }}
                            </h2>
                            <p v-if="delivery.body" class="mt-1 line-clamp-3 text-xs leading-5 text-white/60">
                                {{ delivery.body }}
                            </p>
...
text, quota_count = quota_pattern.subn(quota_replacement, text, count=1)
if quota_count != 1:
    raise SystemExit(f"Expected one quota presentation block, found {quota_count}")
...
for old, new in button_replacements:
    if old not in text:
        raise SystemExit(f"Missing Feed button anchor: {old[:80]!r}")
    text = text.replace(old, new, 1)

for token in [
    "campaign_name: string | null;",
    "const advancingDelivery = ref(false);",
    "aucun second quota ne sera consommé.",
    "delivery.brand_name || delivery.campaign_name",
    "delivery.cta_label || 'Découvrir'",
    ':disabled="advancingDelivery"',
]:
    if token not in text:
        raise SystemExit(f"Missing final Feed token: {token}")
